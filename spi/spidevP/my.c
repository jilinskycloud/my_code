...
char * spi_read(int add1,int add2,int nbytes,int file)
{
  ...
  //printf("read_env: %02x %02x %02x\n", buf[0], buf[1], buf[2]);
  //printf("read_ret: %02x %02x %02x %02x\n", buf2[0], buf2[1], buf2[2], buf2[3]);
  printf("read_ret: %02x \n", buf2[0]);
  com_serial=1;
  failcount=0;
  return buf2;
...
void main(){
   ...
   for(int xy=1;xy>0;xy++){
     //spi_write(0xE6,0x0E,2,buf,file);
     buffer=(char *)spi_read(0xE6,0x0E,6,file); //reading the address 0xE60E
